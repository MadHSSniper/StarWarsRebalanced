...
public class AlderaanSystem implements SectorGeneratorPlugin {

    @Override
    public void generate(SectorAPI sector) {

        StarSystemAPI system = sector.createStarSystem("Alderaan");
        system.setBackgroundTextureFilename("graphics/backgrounds/background4.jpg");

        PlanetAPI star = system.initStar("anderaanStar", StarTypes.YELLOW, 400f, 200);
        star.setName("Alderaan");
        system.setLightColor(new Color(255, 250, 250));

        system.getLocation().set(SW_SystemCalculator.calcX(1942), SW_SystemCalculator.calcY(-89));

        PlanetAPI alderaan = system.addPlanet("AlderaanPlanet", star, "Alderaan", "coruscant", 25, 122, 6000, 365);
        alderaan.setFaction("galacticempire");
        ...
        JumpPointAPI alderaanJumpPoint = Global.getFactory().createJumpPoint("AlderaanJumpPoint", "Alderaan Jump-Point");
        OrbitAPI orbit = Global.getFactory().createCircularOrbit(star, 42, 6000, 365);
        alderaanJumpPoint.setOrbit(orbit);
        alderaanJumpPoint.setRelatedPlanet(alderaan);
        alderaanJumpPoint.setStandardWormholeToHyperspaceVisual();
        system.addEntity(alderaanJumpPoint);
