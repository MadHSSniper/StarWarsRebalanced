...
public class DantooineSystem implements SectorGeneratorPlugin {

    @Override
    public void generate(SectorAPI sector) {

        StarSystemAPI system = sector.createStarSystem("Dantooine");
        system.setBackgroundTextureFilename("graphics/backgrounds/background4.jpg");

        PlanetAPI star = system.initStar("dantooineStar", StarTypes.YELLOW, 400f, 200);
        star.setName("Dantooine");
        system.setLightColor(new Color(255, 250, 250));

        system.getLocation().set(SW_SystemCalculator.calcX(14), SW_SystemCalculator.calcY(8379));

        PlanetAPI dantooine = system.addPlanet("DantooinePlanet", star, "Dantooine", "coruscant", 25, 122, 6000, 365);
        dantooine.setFaction("galacticempire");
        ...
        JumpPointAPI dantooineJumpPoint = Global.getFactory().createJumpPoint("DantooineJumpPoint", "Dantooine Jump-Point");
        OrbitAPI orbit = Global.getFactory().createCircularOrbit(star, 42, 6000, 365);
        dantooineJumpPoint.setOrbit(orbit);
        dantooineJumpPoint.setRelatedPlanet(dantooine);
        dantooineJumpPoint.setStandardWormholeToHyperspaceVisual();
        system.addEntity(dantooineJumpPoint);
